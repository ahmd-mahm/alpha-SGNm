function [X,I,DAind,SRind,epsilon] = asgn_sr(alpha,R,n,tau_max,epsilon,varargin)
...
if nargin<4
    error('Provide atleast: alpha, R, n and tau_max')
elseif nargin==4
    epsilon=icdf('Stable',1-2.5/100,alpha,0,sqrt(R(1,1)),0);
elseif nargin==5
    epsilon=icdf('Stable',1-epsilon/(2*100),alpha,0,sqrt(R(1,1)),0); %evaluate epsilon
end
...
if m~=0
    ...
    if ~isequal(alphaPrev,alpha)
        ...
        load(['vr_alpha=',num2str(alpha),'.mat']) % Gets the variables: 'vJoint','Nmax','Nmin','res'
        alphaPrev=alpha;
    ...
    if n<=m
        ...
        for i = m+1:n
            ...
            switch state % Adds the surface reflection to the aSGN regression equation
                case 's1'
                    if i==m+1
                        width=0;
                        imp_det=false;
                    end
                    if rms(X(i-m+1:i)) >= epsilon
                        width=width+1;
                        imp_det=true;
                    elseif imp_det==true
                        tau=round(rand(1,1)*tau_max)+m;
                        %tau=100;
                        ind=i-width:i;
                        state='s2';
                    end
                ...
                case 's2'
                    if i>=tau+ind(1) && i<=tau+ind(end)
                        X(i)=(X(i-tau))/(1.^alpha);
                    elseif rms(X(i-m+1:i)) < epsilon && i > tau+ind(end)
                        SRind([ind(1),ind(end)]+tau)=X([ind(1),ind(end)]+tau);
                        DAind([ind(1),ind(end)])=X([ind(1),ind(end)]);
                        width=0;
                        imp_det=false;
                        state='s1';
                    end
...
% figure
% plot(X); hold on; plot(find(S~=0),S(S~=0),'v','MarkerFaceColor','auto')
% plot(find(D~=0),D(D~=0),'s','MarkerFaceColor','auto')
% plot([1,length(X)],[thresh,thresh],'--k',[1,length(X)],-[thresh,thresh],'--k')
% X_env=envelope(X,4,'rms');
% plot(X_env)